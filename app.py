            ...
            for fn_col, link_col in pairs:
                ...
                if pd.notna(filename) and pd.notna(link):
                    try:
                        ...
                        # 1. If the aspect ratio is nearly exactly the box and the image is big enough, fill box directly.
                        AR_TOLERANCE = 0.02  # adjust for precision as needed
                        ...
                        if is_proportional and is_big_enough:
                            # Fill the box completely, no margin needed.
                            img_resized = img.resize((width, height), Image.LANCZOS)
                            new_img = img_resized
                        else:
                            # Add margin if needed: fit image inside content area then center on background.
                            if img_ratio > content_box_ratio:
                                # Fit width to content area
                                new_width = content_width
                                new_height = int(content_width / img_ratio)
                            else:
                                # Fit height to content area
